...
public sealed class NPK
{
    ...
    private static readonly int LayerCount = 3;
    ...
    public void Initialise(float originalValue, NPK? nutrients = null)
    {
        ...
        {
            this[key, Original] = GameMath.Clamp(originalValue, 0f, Settings.MaxFertilizedNutrient);
            this[key, Current] = nutrients is null
            ?   this[key, Original]
            :   GameMath.Clamp(nutrients[key], 0f, Settings.MaxFertilizedNutrient);
    ...
    private void AddOverTime(char key, float value) => 
        this[key, OverTime] += Math.Min(Math.Max(0f, Settings.MaxFertilizedNutrient - this[key, OverTime]), value);
    ...
    public bool Tick(double totalHours)
    {
        ...
        foreach (char key in Keys)
        {
            ...
            if (this[key, Current] < this[key, Original])
                this[key, Current] = Math.Min(this[key, Original], this[key, Current] + Settings.FertilityRecoveryPerTick * (float)hoursPassed / 3f);

            if (this[key, OverTime] > 0)
            {
                float release = Math.Min(Settings.FertilizerReleasePerTick * (float)hoursPassed / 3f, this[key, OverTime]);
                this[key, Current] = Math.Min(Settings.MaxFertilizedNutrient, this[key, Current] + release);
