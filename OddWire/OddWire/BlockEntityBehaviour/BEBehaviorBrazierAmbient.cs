...
namespace OddWire.GameContent
{
    public class BEBehaviorBrazierAmbient : BlockEntityBehavior
    {
        ...
        public override void Initialize(ICoreAPI api, JsonObject properties)
        {
            ...
            if (Blockentity.Api.Side == EnumAppSide.Client && bebrazier != null)
            {
        ...
        private void OnTick3s(float dt)
        {
            if (MusicActive)
            {
                if (!fadingOut && track != null && track.IsActive && (!bebrazier.IsBurning || !HasNearbySittingPlayer || !IsNight))
                {
                    fadingOut = true;
